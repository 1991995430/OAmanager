...
<div id="showdiv" style="width: 60%; margin: 0 auto; height:500px; border: 1px solid #999; display: none; position: absolute; top: 20%; left: 20%; z-index: 3; background: #fff">
	...
	<script type="text/javascript">
        // 将最新的3条数据展示到页面
        $(function(){
			$.get('${pageContext.request.contextPath}/getNoticeListOrder',function (data) {
                $(data).each(function (i,ele) {
					$('#notices').after('<li class="ue-clear">\n' +
                        '<span>'+ele.ndate+'</span>&nbsp;&nbsp;&nbsp;<a href="javascript:showWindow('+ele.nid+')"class="notice-title">'+ele.ntitle+'</a>\n' +
                        '</li>');
                })
            },'json');
        });
        ...
        function showWindow(obj) {
            //nid数据
			$.get('${pageContext.request.contextPath}/getNoticeListOrderByNid?nid='+obj,function (data) {
				//数据填充
