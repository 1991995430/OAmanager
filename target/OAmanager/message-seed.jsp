...
<script type="text/javascript">
    ...
    $(function () {
        //获取收件人列表
        $.post('${pageContext.request.contextPath}/getEmpoyeePosition',function (data) {
            $(data).each(function (index,ele) {
                //alert(ele.eid);
                $('#recvp').append('<option value="'+ele.eid+'">'+ele.ename+'--'+ele.position.name+'</option>');
            });
        },'json');

        //提交表单
        $('#save').click(function () {
			var param = $('#form12').serialize();
			//alert(param);
			$.post('${pageContext.request.contextPath}/sendMsg',param,function (data) {
				alert(data.msg);
            },'json');
        });
    });
...
<form name="form2" id="form12"  method="post">
